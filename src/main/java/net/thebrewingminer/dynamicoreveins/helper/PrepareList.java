...
public final class PrepareList {
     ...
     *          The DynamicOreVeinSampler passes over the vein list and tests for the first succeeding vein config, and shuffling the
    ...
    public static List<OreVeinConfig> getOrShuffleList(DensityFunction.NoisePos functionContext, IVeinCondition.Context veinContext){
        ...
        double rawNoise = shuffleSource.sample(functionContext);                    // Get raw value.
        ...
        if(getActiveDebugSettings().printShuffledList()){
            if (!(cachedShuffleSourceSeed == shuffleSourceSeed)) {
                ...
                System.out.println("[DOV] Final shuffled vein list: " + currentList.stream().map(Identifier::toString).collect(Collectors.joining(", ")));
