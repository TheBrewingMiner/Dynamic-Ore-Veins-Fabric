...
public class VeinSettingsConfig {
    ...
    private final ConcurrentHashMap<RegistryKey<World>, DensityFunction> mappedDensityFunctions = new ConcurrentHashMap<>();    // Caches the mapped functions per dimension.
    ...
    public static final Codec<VeinSettingsConfig> CODEC = RecordCodecBuilder.create(configInstance -> configInstance.group(
            ...
            DensityFunction.FUNCTION_CODEC.fieldOf("shuffle_source").forGetter(config -> config.shuffleSource),
    ...
    public DensityFunction getOrMapFunction(IVeinCondition.Context context){
        return mappedDensityFunctions.computeIfAbsent(context.dimension(), __ -> this.shuffleSource.apply(new LegacyNoiseDensityFunctionVisitor(context)));
    }
