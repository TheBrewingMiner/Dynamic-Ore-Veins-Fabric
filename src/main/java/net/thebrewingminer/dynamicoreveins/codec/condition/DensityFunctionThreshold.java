...
import net.thebrewingminer.dynamicoreveins.helper.LegacyNoiseDensityFunctionVisitor;
...
public class DensityFunctionThreshold implements IVeinCondition{
    ...
    private final Map<RegistryKey<World>, DensityFunction> mappedDensityFunctions = new ConcurrentHashMap<>();      // Caches mapped function per dimension per condition.
    ...
    public static final Codec<DensityFunctionThreshold> DENSITY_FUNCTION_THRESHOLD_CODEC = RecordCodecBuilder.create(instance -> instance.group(
            DensityFunction.FUNCTION_CODEC.optionalFieldOf("input").forGetter(config -> Optional.ofNullable(config.function)),
    ...
    protected double computeValue(DensityFunction densityFunction, Context context){
        ...
        return (densityFunction.sample(new DensityFunction.UnblendedNoisePos(x, y, z)));
    }
    ...
    public boolean test(Context context){
        if (!(context.chunkGenerator() instanceof NoiseChunkGenerator)) return false;  // Guard clause. Chunk generator type should be implicit but just to be sure.
        ...
        }                                                                                           // allowed to be null in three cases in DynamicOreSampler.

        // Wire and cache the provided function.
        DensityFunction wiredFunction = mappedDensityFunctions.computeIfAbsent(context.dimension(), __ -> this.function.apply(new LegacyNoiseDensityFunctionVisitor(context)));
    ...
    public DensityFunction getOrMapFunction(Context context) {
        ...
        return mappedDensityFunctions.computeIfAbsent(context.dimension(), __ -> {
            ...
            return this.function.apply(new LegacyNoiseDensityFunctionVisitor(context));
        });
    }
